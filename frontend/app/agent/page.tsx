...
const KeyRevealModal: FC<KeyRevealModalProps> = ({
	rawKey,
	prefix,
	label,
	onClose,
			...
			<div className="bg-[#141414] border border-neutral-700 rounded-2xl w-full max-w-lg p-6 font-display">
				<div className="flex items-start justify-between mb-4">
					...
					<button
						onClick={onClose}
						disabled={!confirmed}
						className="text-neutral-500 hover:text-white disabled:opacity-30 disabled:cursor-not-allowed transition-colors ml-4 mt-0.5"
						type="button"
					>
						<svg
							className="w-5 h-5"
				...
				<div className="flex items-start gap-2 bg-amber-500/10 border border-amber-500/30 rounded-xl px-4 py-3 mb-4">
					...
					<p className="text-xs text-amber-300 leading-relaxed">
						This is the only time this key will be shown. Copy it and paste it
						into your agent platform immediately.
					</p>
				</div>

				<div className="bg-[#0a0a0f] border border-neutral-700 rounded-xl px-4 py-3 mb-4 flex items-center gap-3">
					<code className="text-sm text-green-400 font-mono flex-1 break-all select-all leading-relaxed">
						{rawKey}
					</code>
					<button
						onClick={copyKey}
						className="shrink-0 px-3 py-1.5 rounded-lg bg-neutral-800 hover:bg-neutral-700 text-xs font-medium transition-colors"
						type="button"
					>
						{copied ? (
				...
				<label className="flex items-center gap-3 cursor-pointer mb-5">
					...
					<span className="text-sm text-neutral-300">
						I have copied and saved the key safely
					</span>
				</label>
				...
				<button
					onClick={onClose}
					disabled={!confirmed}
					className="w-full py-2.5 rounded-xl font-medium text-sm transition-all bg-orange-500 text-white hover:bg-orange-600 disabled:opacity-40 disabled:cursor-not-allowed"
					type="button"
				>
					Done
				</button>
...
const SecretRevealModal: FC<SecretRevealModalProps> = ({
	secret,
	webhookId,
	url,
	onClose,
		...
		<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
			<div className="bg-[#141414] border border-neutral-700 rounded-2xl w-full max-w-lg p-6 font-display">
				...
				<div className="flex items-start gap-2 bg-amber-500/10 border border-amber-500/30 rounded-xl px-4 py-3 mb-4">
					...
					<p className="text-xs text-amber-300 leading-relaxed">
						Save this signing secret now. You'll use it to verify incoming
						webhook payloads. It will not be shown again.
					</p>
				</div>
				...
				<p className="text-xs text-neutral-500 mb-4">
					Webhook ID:{" "}
					<span className="font-mono text-neutral-400">{webhookId}</span>
				</p>
...
const AgentPage: FC = () => {
	const { publicKey, walletId, userId } = usePrivyWallet();
	...
	const handleEnableDelegation = async () => {
		...
		try {
			await addSigners({
				address: publicKey,
				signers: [{ signerId: "o188vko4g7y7965meu4gnxry", policyIds: [] }],
			});
			...
			toast(
				err instanceof Error ? err.message : "Failed to enable delegation",
				"error",
			);
		} finally {
			setDelegating(false);
	...
	const fetchKeys = useCallback(async () => {
		...
		try {
			...
			setKeyError(
				err instanceof ApiError ? err.message : "Failed to load keys",
			);
		} finally {
	...
	const handleGenerate = async () => {
		...
		try {
			...
			toast(
				err instanceof ApiError ? err.message : "Failed to generate key",
				"error",
			);
		} finally {
			setGenerating(false);
	...
	const handleTestWebhook = async (webhookId: string) => {
		...
		try {
			...
			toast(
				err instanceof ApiError ? err.message : "Test delivery failed",
				"error",
			);
		} finally {
		...
		<div className="min-h-screen bg-[#0a0a0f] text-white font-display">
						...
						<div className="flex items-start justify-between gap-4">
							<div className="min-w-0">
								<div className="flex items-center gap-2 mb-1">
									<div
										className={`w-2 h-2 rounded-full shrink-0 ${isDelegated ? "bg-green-500" : "bg-neutral-600"}`}
									/>
					...
					<div className="bg-[#141414] border border-neutral-700 rounded-2xl p-5 mb-3">
						...
						<div className="flex gap-3">
							...
							<button
								...
								disabled={generating}
								className="px-5 py-2.5 rounded-xl bg-orange-500 hover:bg-orange-600 text-white text-sm font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed shrink-0"
								type="button"
							>
						...
						<p className="text-xs text-neutral-500 mt-2">
							The raw key is shown exactly once — paste it into your agent
							immediately.
						</p>
					</div>
					...
					{loadingKeys ? (
						...
						<div className="space-y-2">
							{activeKeys.map((key) => (
								<div
									key={key.id}
									className="bg-[#141414] border border-neutral-700 rounded-2xl px-5 py-4 flex items-center justify-between gap-4"
								>
									<div className="min-w-0">
										<div className="flex items-center gap-2 mb-0.5">
											...
											<span className="text-xs text-neutral-500 font-mono bg-neutral-800 px-2 py-0.5 rounded-md shrink-0">
												{key.key_prefix}…
											</span>
										</div>
									...
									<button
										...
										className="shrink-0 px-3 py-1.5 rounded-lg text-xs font-medium text-red-400 border border-red-400/30 hover:bg-red-400/10 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
										type="button"
									>
					...
					{revokedKeys.length > 0 && (
						<div className="mt-4 space-y-2">
							{revokedKeys.map((key) => (
								<div
									key={key.id}
									className="border border-neutral-800 rounded-xl px-5 py-3 flex items-center justify-between opacity-40"
								>
				...
				<section>
					...
					<div className="bg-[#141414] border border-neutral-700 rounded-2xl divide-y divide-neutral-800">
						...
						{!sessionKey ? (
							<div className="px-5 py-4">
								<p className="text-sm font-medium text-white mb-1">
									Unlock webhook management
								</p>
								<p className="text-xs text-neutral-500 mb-3">
									Paste one of your API keys to manage webhooks. We never store
									it — it's only used for this session.
								</p>
								<div className="flex gap-3">
									<input
										type="text"
									...
									<button
										...
										className="px-5 py-2.5 rounded-xl bg-neutral-800 hover:bg-neutral-700 text-white text-sm font-medium transition-colors shrink-0"
										type="button"
									>
										Unlock
									</button>
							...
							<>
								...
								<div className="px-5 py-4">
									...
									<p className="text-xs text-neutral-500 mb-3">
										Hastrology POSTs to this URL when an event fires — no
										polling needed.{" "}
										<span className="text-neutral-400">
											...
											, paste it here, then click Test — you&apos;ll see the
											live ping instantly. In production, use your agent
											server&apos;s URL.
										</span>
									</p>
									<input
											...
											>
												<input
													...
													onChange={(e) =>
														setSelectedEvents(
															e.target.checked
																? [...selectedEvents, ev.value]
																: selectedEvents.filter((x) => x !== ev.value),
														)
													}
									...
									{loadingWebhooks ? (
										...
										<div className="space-y-3">
											{activeWebhooks.map((wh) => (
												<div
													key={wh.id}
													className="bg-[#0a0a0f] border border-neutral-800 rounded-xl px-4 py-3"
												>
													...
													<div className="flex flex-wrap gap-1.5">
														{wh.events.map((ev) => (
															<span
																key={ev}
																className="text-xs font-mono bg-neutral-800 text-neutral-400 px-2 py-0.5 rounded-md"
															>
																{ev}
															</span>
														))}
			...
			{revealSecret && (
				<SecretRevealModal
					...
					webhookId={revealSecret.webhookId}
